...
@Model.register("pointer_network")
class LSTMPointerNet(Model):
    PAD_CHAR = -1
    ...
    def forward(self, attn_mem, lstm_in, mem_sizes=None, memory_mask=None):
        ...
        #print('MEMORY_MASK', memory_mask[0])                
        #print('ATTN_MEM', attn_mem[0])
        #print('ATTN_FEAT', attn_feat[0])
        #print('HOP_FEAT', hop_feat[0])
        ...
        for _ in range(self._n_hop):
            query = LSTMPointerNet.attention(
                hop_feat, query, self._hop_v, self._hop_wq, mem_sizes, memory_mask)
        #print('QUERY', query)            
    ...
    def extract(self, attn_mem, mem_sizes, k, mask=None, beam_size = 5):
        ...
        if beam_size <= 1:
            extracts = torch.stack(extracts, dim=0)
    ...
    @staticmethod
    def attention(attention, query, v, w, mem_sizes, mask=None, attn_feat=None):
        """ attention context vector"""

        if attn_feat is None:
            attn_feat = attention
        
        score = LSTMPointerNet.attention_score(attn_feat, query, v, w)
...
@Model.register("pointer_network_summarizer")
class PtrExtractSumm(Model):
